...
class BookmarkChangeService {

    private BookmarkRepository repository;

    BookmarkChangeService(BookmarkRepository repository) {
        super();
     ...
     */
    Bookmark change(BookmarkId id, BookmarkName name, BookmarkDescription description, BookmarkURL url) {
        checkName(id, name);
        checkURL(id, url);
